...
fn test_resize_image() {
    ...
    let resized_img = resize_image(&img, 200, 200);
    assert!(resized_img.is_ok());
    assert_eq!(resized_img.unwrap().len(), 200 * 200 * 4);

    let resized_img = resize_image(&img, 50, 50);
    assert!(resized_img.is_ok());
    assert_eq!(resized_img.unwrap().len(), 50 * 50 * 4);

    let resized_img = resize_image(&img, 100, 100);
    assert!(resized_img.is_ok());
    assert_eq!(resized_img.unwrap().len(), 100 * 100 * 4);

    let resized_img = resize_image(&img, 200, 150);
    assert!(resized_img.is_ok());
    assert_eq!(resized_img.unwrap().len(), 200 * 150 * 4);

    let resized_img = resize_image(&img, 150, 200);
    assert!(resized_img.is_ok());
